...
void ParticleSystem::setVelocity(ofVec3f vel){
    for (int i = 0; i < particles.size(); i++) {
...
ImpulseForce::ImpulseForce(float magnitude) {
    this->magnitude = magnitude;
    //applyOnce = true;
}
...
void ImpulseForce::updateForce(Particle * particle) {
    ...
    //
    //ofVec3f dir = ofVec3f(ofRandom(-1, 1), ofRandom(-height/2.0, height/2.0), ofRandom(-1, 1));
    particle->forces += this->dir.getNormalized() * magnitude;
}
...
bool ParticleSystem::onTallGround(ParticleSystem* ground){
    if ((particles[0].position.x-particles[0].width/2) < ground->particles[0].position.x + ground->particles[0].width/2){
        if ((particles[0].position.z-particles[0].depth/2) < ground->particles[0].position.z + ground->particles[0].depth/2){
            if((particles[0].position.y-particles[0].height/2) < ground->particles[0].position.y + ground->particles[0].height/2) {
                return true;
            }
        }
    }
